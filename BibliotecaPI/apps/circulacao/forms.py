...
class EmprestimoForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        instance.data_prevista_devolucao = (
            timezone.localdate() + timedelta(days=PRAZO_EMPRESTIMO_DIAS)
        )
        if commit:
            instance.save()
        return instance
...
class ReservaBuscaForm(forms.Form):
    ...
    autor = forms.CharField(
        required=False,
        label='Autor',
        widget=forms.TextInput(attrs={'placeholder': 'Ex: Machado de Assis'}),
    )
    isbn = forms.CharField(
        required=False,
        label='ISBN (10 ou 13)',
        widget=forms.TextInput(attrs={'placeholder': 'Ex: 9788544001820'}),
    )
