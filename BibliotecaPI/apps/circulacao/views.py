    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if reserva_id:
            ...
            if not self.reserva_origem.exemplar_id:
                messages.warning(request, 'A reserva ainda não possui exemplar separado para retirada.')
...
def registrar_devolucao(request, pk):
    ...
    emprestimo = get_object_or_404(
        Emprestimo.objects.select_related('exemplar__livro', 'usuario'), pk=pk
    )
...
# ---------------------------------------------------------------------------
# Ações rápidas para usuários comuns
# ---------------------------------------------------------------------------
...
def solicitar_emprestimo(request, livro_id):
    ...
    try:
        ...
        messages.success(
            request,
            f'Solicitação registrada. Protocolo {reserva.protocolo}. Retire até {retirada} no balcão.',
        )
    return redirect('circulacao:emprestimo_list')


# ---------------------------------------------------------------------------
# APIs JSON para Select2
# ---------------------------------------------------------------------------
...
def api_buscar_exemplares(request):
    ...
    if q:
        exemplares = exemplares.filter(
            Q(livro__titulo__icontains=q) | Q(codigo_tombo__icontains=q)
        )
